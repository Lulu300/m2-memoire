...
CouchBase est une solution open source sous la licence Apache 2, développé en C++, Erlang et Go par CouchBase.
...
HBase se focalise sur la haute disponibilité des données stockées, il propose donc des performances en lecture très élevée. Par contre, il ne propose pas de cohérence des données.
...
Cassandra est une solution native, il n'a pas besoin de librairies externes. Contrairement à HBase qui se focalise sur la disponibilité des données, Cassandra lui se focalise sur la cohérence des données. Cela a pour impact de le rendre moins performant que son concurrent pour la lecture des données. En contrepartie, il permet de pouvoir répliqué son stockage sur différentes machines à tout moment.
