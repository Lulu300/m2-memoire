...
Kafka utilise le système de communication du Publisher(Producer) / Subscriber en utilisant un élément unique, appelé broker. Afin de faciliter la mise à l'échelle et le partitionnement des données, il est possible d'instancier autant que broker que l'on souhaite afin d'augmenter le débit et la résilience du produit.
...
Apache Nifi est une solution ETL/ELT open source sous la licence Apache 2 écrite en Java et développée par la fondation Apache. 
...
InfluxDB est une solution Open Source développée par InfluxData sous la licence MIT et écrit en Go.
...
JanusGraph propose lui aussi une mise à l'échelle pour la lecture des données et avec des performances similaires par rapport à son concurrent. Mais il ne s'arrête pas là, grâce à son utilisation de brique extérieure (HBase ou Cassandra), il propose aussi une mise à l'échelle pour l'écriture des données. 
...
Elasticsearch fait partie de la suite elastic qui propose aussi un outil d'analyse et de visualisation des données nommées Kibana (voir partie {\ref{kibana}}). Kibana et Elasticsearch sont entièrement compatibles, plus précisément Kibana s'intègre uniquement avec Elasticsearch.
...
CouchDB est une solution open source sous la licence Apache 2, développé en Erlang par la fondation Apache.

CouchDB propose une solution de réplication des données très avancée. Cela permet de mettre en place la réplication très rapidement. CouchDB s'oriente vers la mise en place de données cohérente. Cela a pour impact de réduire les performances en lecture et en écriture.

\subsubsection{MongoDB}

MongoDB est une solution open source sous la licence OSI, développé en C++, Go, JavaScript et Python.

MongoDB contrairement à son concurrent, s'oriente vers un mode de haute disponibilité afin de garantir des performances maximales. Cela l'empêche donc d'assurer correctement la réplication des données.
...
HBase est une solution open source sous la licence Apache 2, développée en Java par la fondation Apache.

La figure {\ref{wide-column-hbase}} représente un exemple de la manière dont sont stockées les données dans HBase.

\begin{figure}[!h]
	\centering 
	\includegraphics[scale=0.45]{img/wide-column-hbase.png}
	\captionsource{Schéma du principe de stockage de HBase}{http://www.alliedc.com/apache-cassandra-database-4-problems-that-cassandra-developers-administrators-face/}
	\label{wide-column-hbase}
\end{figure}

HBase utilise HDFS (voir partie {\ref{hdfs}}) comme solution de stockage et Zookeeper afin de gérer la mise à l'échelle.

HBase se focalise sur la cohérence des données stockées, mais peine un peu plus au niveau des écritures par rapport à ces concurrents.

\subsubsection{Cassandra}
\label{cassandra}

Cassandra est une solution open source sous la licence Apache 2, développée en Java par la fondation Apache.

Cassandra est une solution native, il n'a pas besoin de librairies externes. Contrairement à HBase qui se focalise sur la cohérence des données, Cassandra lui se focalise sur la disponibilité des données. En contrepartie, il permet d'avoir des taux d'écriture supérieurs à HBase.

La figure {\ref{wide-column-cassandra}} représente un exemple de la manière dont sont stockées les données dans Cassandra.
...
HBase et Cassandra partagent la même base avec un format de stockage de données très similaires, mais leurs cas d'utilisation sont complètement opposés.
...
Kibana est une solution open source sous la licence Apache 2 écrite en JavaScript et développée par Elastic.

Kibana fait partie de la suite Elastic, il est conçu spécialement pour permettre de faire la visualisation et de l'analyse des données stockées sur Elasticsearch.

Elasticsearch propose la création de Dashboard qui est constitué de visualisations. Kibana propose plusieurs types de visualisations, les classiques graphes linéaires, camembert, etc. Il propose également des visualisations géographiques, des interfaces de séries temporelles, un module d'étude de graph et pour finir de l'apprentissage automatique pour faire de la détection d'anomalies en temps réel.

\subsection{Banana}
\label{banana}

Banana est une solution open source sous la licence Apache 2 écrite JavaScript et développé par lucidworks.

Banana est une solution d'analyse et de visualisation de données spécialement conçue pour fonctionner avec Apache Solr. Banana ressemble énormément à Kibana, cela s'explique par le fait que c'est un fork de Kibana adapté pour fonctionner avec Apache Solr. Les seules différences avec Kibana sont dans la différence de direction évoquée lors de la présentation d'Apache Solr.

\subsection{Grafana}

Grafana est une solution open source sous la licence Apache 2 écrite en Go et développé par Grafana Lab.
...
Apache Oozie est une solution open source sous la licence Apache 2 écrite en Java et développée par Apache.
...
Une solution beaucoup plus basique et qui existe depuis longtemps, est l'utilisation de cron tab sous Linux. Cette solution propose uniquement de lancer une tâche tous les x temps, mais avec l'utilisation des messages broker, il n'est pas forcément nécessaire de lancer une suite de tâches. En effet, si les messages brokers permettent de déclencher des évènements afin de récupérer les données, les envoyer au logiciel de traitement de données puis les stocker dans la base de données sélectionnée, la seule tâche restante est la génération d'un rapport. L'utilisation de cron tab est suffisante pour ce genre de tâche et ne nécessite aucune installation, car elle est installée avec Linux.
